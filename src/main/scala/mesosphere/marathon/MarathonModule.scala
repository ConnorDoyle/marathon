...
object ModuleNames {
  ...
  final val NAMED_STORE_TASK = "TaskStore"
}
...
class MarathonModule(conf: MarathonConf, http: HttpConf, zk: ZooKeeperClient)
  ...
  // persistence fucntionality ----------------

  @Provides
  @Singleton
  def provideTaskFailureRepository(
    @Named(ModuleNames.NAMED_STORE_TASK_FAILURES) store: EntityStore[TaskFailure],
  ...
  def provideTaskRepository(
    @Named(ModuleNames.NAMED_STORE_TASK) store: EntityStore[MarathonTaskState],
  ...
  @Named(ModuleNames.NAMED_STORE_TASK)
  @Provides
  @Singleton
